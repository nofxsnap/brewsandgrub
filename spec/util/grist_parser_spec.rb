...
# Caution:  this test uses live data, has an external dependency until I can figure out
# how to do the local file correctly

RSpec.describe GristParser do
  SPEC_BREWERY_EVENT_HOURS = "4:00pm-9:00pm"

  before(:all) do
    @brewery = create(:grist)
  ...
  before(:each) do
    ...
    # @brewery.update_attribute(:remote_schedule_endpoint, "#{Rails.root}/spec/util/dataz/index.html?date=4%2F6%2F2017")
  end

  describe '#test_the things' do
    it "should get food truck for the day and create a new food truck" do
      today = Date.new(2017,4,6)
      ...
      expect(@brewery.event_hours).not_to be(nil)
    end

    # For now, skipping this logic since we know the code for adding a truck works in other places.
